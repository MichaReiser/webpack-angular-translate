...
var isCommentedWithSuppressError = require("../../dist/js/translate-visitor").isCommentedWithSuppressError;
...
describe("TranslateVisitor", function () {
    ...
    describe("$translate", function () {
        ...
        it("extracts the translation with it's id and default text from a $translate call with a four argument", function () {
            var translateCall = b.callExpression($translate, [b.literal("test"), b.identifier("undefined"), b.identifier("undefined"), b.literal("Test")]);
            translateCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";
        ...
        it("extracts all translation with their ids for a $translate call with an array of translation ids", function () {
            var translateCall = b.callExpression($translate, [b.arrayExpression([b.literal("test"), b.literal("test2")])]);
            translateCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";
        ...
        it("emits an error if the default text is not a literal", function () {
            var translateCall = b.callExpression($translate, [b.literal("test"), b.literal(null), b.literal(null), b.identifier("test")]);
            translateCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";

            visitor.visit(translateCall);

            sinon.assert.calledWith(loaderContext.emitError, "Illegal argument for call to $translate: The default text should be a string literal. If you have registered the translation manually, you can use a /* suppress-dynamic-translation-error: true */ comment in the block of the function call to suppress this error. (test.js:1:1).");
        });
        ...
        it("suppress the call needs at least one argument error if block contains 'suppress-dynamic-translation-error: true' comment", function () {
            ...
            lineComment.loc = { start: { line: 1, column: 1 }, end: { line: 1, column: 38}};
            ...
            root.loc = { start: { line: 1, column: 1 }, end: { line: 2, column: 12 }};

            visitor.visit(root);

            assert.notOk(loaderContext.emitError.called, "No error has been emitted");
            assert.notOk(loaderContext.registerTranslation.called, "No translation has been registered")
        });
        ...
        it("suppress the default value needs to be a literal error if block contains 'suppress-dynamic-translation-error: true' comment", function () {
            ...
            var translateCall = b.callExpression($translate, [b.literal("test"), b.literal(null), b.literal(null), b.identifier("defaultText")]);
            translateCall.loc = { start: { line: 2, column: 1 } };
            loaderContext.resource = "test.js";
    ...
    describe("i18n.registerTranslation", function () {
        ...
        beforeEach(function () {
            ...
            registerTranslation = b.memberExpression(i18n, b.identifier("registerTranslation"));
        });

        it("extracts the translation with it's id from a i18n.registerTranslation call with a single argument", function () {
            var registerTranslationCall = b.callExpression(registerTranslation, [b.literal("test")]);
            ...
            assert.equal(ast.value, "test", "has removed the registerTranslation call with the translation id");
        });
        ...
        it("extracts the translation with it's id and default text from a i18n.registerTranslation call with a two arguments", function () {
            var registerTranslationCall = b.callExpression(registerTranslation, [b.literal("test"), b.literal("default Text")]);
            registerTranslationCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";
        ...
        it("emits an error if called without arguments", function () {
            ...
            sinon.assert.calledWith(loaderContext.emitError, "Illegal argument for call to 'i18n.registerTranslation'. The call requires at least the 'translationId' argument that needs to be a literal (test.js:1:1).");
        });

        it("emits an error if the translation is not a literal", function () {
            var registerTranslationCall = b.callExpression(registerTranslation, [b.identifier("test")]);
            registerTranslationCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";
        ...
        it("emits an error if the default text is not a literal", function () {
            var registerTranslationCall = b.callExpression(registerTranslation, [b.literal("test"), b.identifier("defaultText")]);
            registerTranslationCall.loc = { start: { line: 1, column: 1 } };
            loaderContext.resource = "test.js";

            visitor.visit(registerTranslationCall);

            sinon.assert.calledWith(loaderContext.emitError, "Illegal argument for call to i18n.registerTranslation: the default text has to be a literal (test.js:1:1).");
        });
    });
    ...
    describe("i18n.registerTranslations", function () {
        ...
        beforeEach(function () {
            i18n = b.identifier("i18n");
            registerTranslations = b.memberExpression(i18n, b.identifier("registerTranslations"));
        });
    ...
    describe("isCommentedWithSuppressError", function () {
        ...
        it("returns false for a program without a comment", function () {
            ...
            program.loc = { start: { line: 1, column: 1}, end: {line: 1, column: 12 } };
        ...
        it("returns true for a program with a comment", function () {
            ...
            program.loc = { start: { line: 1, column: 1}, end: {line: 2, column: 12 } };
        ...
        it("returns true if the containing program contains a comment", function () {
            ...
            var expression = b.expressionStatement(b.callExpression(b.identifier("$translate"), []));
            expression.loc = { start: { line: 2, column: 1 }, end: { line: 2, column: 12 }};

            var program = b.program([ expression ]);
            program.loc = { start: { line: 1, column: 1}, end: {line: 2, column: 12 } };

            var root = new types.NodePath({ root: program }).get("root");
            assert.ok(isCommentedWithSuppressError(root.get("body").get(0), comments));
        });
        ...
        it("returns true if a parent block contains a comment", function () {
            var lineComment = b.line("suppress-dynamic-translation-error: true");
            lineComment.loc = { start: { line: 2, column: 1 }, end: { line: 2, column: 38}};
            comments.push(lineComment);

            var expression = b.expressionStatement(b.callExpression(b.identifier("$translate"), []));
            expression.loc = { start: { line: 3, column: 1 }, end: { line: 3, column: 12 }};

            var block = b.blockStatement([expression]);
            block.loc = { start: { line: 1, column: 1 }, end: { line: 4, column: 1 }};

            var program = b.program([
                block
            ]);
            program.loc = { start: { line: 1, column: 1}, end: {line: 4, column: 1 } };

            var root = new types.NodePath({ root: program }).get("root");

            assert.ok(isCommentedWithSuppressError(root.get("body").get(0).get("body").get(0), comments));
        });
        ...
        it("returns false if a sibling block contains a comment", function () {
            var expression = b.expressionStatement(b.callExpression(b.identifier("$translate"), []));
            expression.loc = { start: { line: 1, column: 1 }, end: { line: 1, column: 12 }};

            var block = b.blockStatement([expression]);
            block.loc = { start: { line: 2, column: 1 }, end: { line: 4, column: 1 }};

            var lineComment = b.line("suppress-dynamic-translation-error: true");
            lineComment.loc = { start: { line: 3, column: 1 }, end: { line: 3, column: 38}};
            comments.push(lineComment);
            ...
            assert.notOk(isCommentedWithSuppressError(root.get("body").get(0), comments));
        });
    });
});