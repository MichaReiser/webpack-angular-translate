...
describe("StatefulHtmlParserSpecs", function () {
    ...
    describe("<translate>", function () {
        it("uses the element text as translation id", function () {
            ...
            sinon.assert.calledWith(loaderContext.registerTranslation, new Translation("Simple", undefined, {
                resource: "test.html",
                loc: { line: 1, column: 0 }
            }));
        });
        ...
        it("uses the value of the translate-default as defaultText", function () {
            ...
            sinon.assert.calledWith(loaderContext.registerTranslation, new Translation("Simple", "Other default", {
                resource: "test.html",
                loc: { line: 1, column: 0 }
            }));
        });
        ...
        it("translates the attribute and the content of the element if translate-attr is set and the element has non empty content", function () {
            parse("<translate translate-attr-title='Attribute'>Element-Text</translate>");

            sinon.assert.calledWith(loaderContext.registerTranslation, new Translation("Attribute", undefined, {
                resource: "test.html",
                loc: { line: 1, column: 0 }
            }));

            sinon.assert.calledWith(loaderContext.registerTranslation, new Translation("Element-Text", undefined, {
                resource: "test.html",
                loc: { line: 1, column: 0 }
            }));
        });
    ...
    describe("<any translate>", function () {
        ...
        it("translates the attribute and the content of the element if translate-attr is set and the element has non empty content", function () {
            parse("<div translate translate-attr-title='Attribute'>Element-Text</div>");
        ...
        it("translates the attribute and the content of the element if translate-attr is set and the translate attribute has an assigned value", function () {
            parse("<div translate='Element-Text' translate-attr-title='Attribute'></div>");
        ...
        it("emits an error if the value of the translate id is an expression", function () {
            ...
            sinon.assert.calledWith(loaderContext.emitError, "Failed to extract the angular-translate translations from test.html:1:0: The element '<div translate='{{controller.title}}'>...</div>'  in 'test.html' uses an angular expression as translation id ('{{controller.title}}') or as default text ('undefined'), this is not supported. To suppress this error at the 'suppress-dynamic-translation-error' attribute to the element or any of its parents.");
        });
        ...
        it("suppresses the error of a dynamic value in the translation id attribute if the element is attributed with suppress-dynamic-translation-error", function () {
            parse("<div translate='{{controller.title}}' suppress-dynamic-translation-error></div>");
        ...
        it("emits an error if a translation does not have a valid id", function () {
            ...
            sinon.assert.calledWith(loaderContext.emitError, "Failed to extract the angular-translate translations from test.html:2:5: the element uses the translate directive but does not specify a translation id nor has any translated attributes (translate-attr-*). Specify a translation id or remove the translate-directive.");
    ...
    describe("<any translate translate-attr-*>", function () {
        ...
        it("uses the default text from the translate-default-attr-*", function () {
            parse("<div translate translate-attr-title='test' translate-default-attr-title='Default Text'></div>");

            sinon.assert.calledWith(loaderContext.registerTranslation, new Translation("test", "Default Text", {
                resource: "test.html",
                loc: { line: 1, column: 0 }
            }));
        });
        ...
        it("doesn't register a translation for the content of an element attributed with translate-attr", function () {
            parse("<div translate translate-attr-title='test'><span>Test</span></div>");
            ...
            assert.equal(loaderContext.registerTranslation.callCount, 1, "Not translated content is not registered as translation");
        });

        it("emits an error if translate-attr uses an expression", function () {
            parse("<div translate translate-attr-title='{{controller.title}}'></div>");

            sinon.assert.calledWith(loaderContext.emitError, "Failed to extract the angular-translate translations from test.html:1:0: The element '<div translate='' translate-attr-title='{{controller.title}}'>...</div>'  in 'test.html' uses an angular expression as translation id ('{{controller.title}}') or as default text ('undefined'), this is not supported. To suppress this error at the 'suppress-dynamic-translation-error' attribute to the element or any of its parents.");
            assert.notOk(loaderContext.registerTranslation.called);
        });
        ...
        it("suppresses the error if translate-attr is used and the element is attributed with suppress-dynamic-translation-error", function () {
            parse("<div translate translate-attr-title='{{controller.title}}' suppress-dynamic-translation-error></div>");
    ...
    describe("{{ any | translate }}", function () {
        ...
        it("emits an error if the translate filter is not the first in the filter chain", function () {
            ...
            sinon.assert.calledWith(loaderContext.emitError, "Failed to extract the angular-translate translations from test.html:1:0: Another filter is used before the translate filter in the element <root>{{ 'title' | uppercase | translate }}</root>. Add the 'suppress-dynamic-translation-error' to suppress the error (ensure that you have registered the translation manually, consider using i18n.registerTranslation).");
        });
        ...
        it("suppresses the error for a filter with a dynamic value if suppress-dynamic-translate-error is used on the parent element", function () {
            parse("<span suppress-dynamic-translation-error>{{ controller.title | translate }}</span>");
        ...
        it("suppresses the error for a filter chain where translate is not the first filter if suppress-dynamic-translate-error is used on the parent element", function () {
            parse("<span suppress-dynamic-translation-error>{{ 'title' | uppercase | translate }}</span>");
